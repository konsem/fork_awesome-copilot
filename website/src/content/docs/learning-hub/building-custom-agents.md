...
**include-custom-instructions** *(v1.0.86+)*: Set this to `true` to have the custom agent opt into repository instruction files (`AGENTS.md`, `copilot-instructions.md`, `CLAUDE.md`) in addition to its own frontmatter and body. By default, custom agents do **not** automatically inherit repository-wide instruction files, so this is useful when you want an agent to layer its specialized persona on top of your team's existing coding standards rather than replace them:
