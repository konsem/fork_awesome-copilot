...
**include-custom-instructions** *(v1.0.86+)*: By default, custom agents run in isolation from your repository's instruction files. Set `include-custom-instructions: true` in an agent's frontmatter to have it also pick up repository instruction files (`AGENTS.md`, `copilot-instructions.md`, `CLAUDE.md`) alongside its own persona — useful when you want a specialized agent to still respect team-wide coding standards:
