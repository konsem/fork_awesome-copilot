...
**Auto routing tier startup defaults** *(v1.0.87+)*: Organizations can now set a default routing tier for the Auto model — applied automatically when a session starts — using a **user default** (which you can override) or a **managed default** (which your organization can mark strict, preventing local overrides). This lets admins standardize on a cost- or capability-appropriate tier organization-wide while still giving individual users flexibility unless a strict policy is in place.
...
**`/config` sidebar** *(v1.0.85+)*: The `/config` command opens a sidebar configuration screen alongside your active conversation, giving you a persistent view of settings without leaving the session:

```
/config
```

**Vim mode** *(v1.0.85+)*: Vim-style modal editing is now available to everyone in the CLI composer. Turn it on with `/vim`, or set `editorMode` to `vim` in your settings, to get familiar Normal/Insert mode keybindings while typing prompts. The current mode is shown while you type.

```
/vim
```

**Transcript view grouping** *(v1.0.85+)*: Set `transcriptView` to `concise` in `/settings` to group tool activity into expandable work summaries, reducing scroll noise in long sessions with many tool calls.
...
**Sandbox network host rules** *(v1.0.85+)*: `/sandbox` now supports adding explicit network host allow/deny rules without replacing your configured upstream proxy — useful when you need to permit access to a specific internal service or block a specific domain while still routing everything else through your existing proxy.

**Local sandbox for shell commands** *(v1.1.23+ Copilot app)*: In the GitHub Copilot app, a new project setting and `/sandbox` command let you run the agent's shell commands in a local sandbox restricted to the session's workspace, mirroring the CLI's sandboxing model for desktop sessions.
