...
**Sidebar configuration screen** *(v1.0.85+)*: Run `/config` to open a dedicated sidebar configuration screen in the CLI, offering a persistent, panel-based alternative to the modal `/settings` dialog for browsing and editing settings while keeping the conversation visible.

**Vim mode** *(v1.0.85+)*: Vim mode is available to everyone. Turn it on with `/vim`, or set `editorMode` to `vim` in your config file, to get modal editing (normal/insert modes, familiar motions and commands) in the message composer. The current mode is shown while you type.
